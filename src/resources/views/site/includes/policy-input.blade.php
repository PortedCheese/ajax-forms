<div class="custom-control custom-checkbox">
    ...
    <label class="custom-control-label" for="privacy_policy{{ $postfix }}">
        ...
        @if (\Illuminate\Support\Facades\Route::has("policy"))
            <a href="#agreementModal" class="{{ isset($class)? $class:'' }}" data-toggle="modal" data-target="#agreementModal">Согласие на обработку персональных данных</a> и принимаю условия <a class="{{ isset($class)? $class:'' }}"  href="{{ route("policy") }}" target="_blank">Политики по обработке персональных данных</a>
        @else
